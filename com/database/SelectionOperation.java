...
public class SelectionOperation {
	...
	public static Table selectionEvaluation(Statement statementObject, HashMap<String, Table> tableObjectsMap)
		...
		List<Column> groupbyElementsList = ((PlainSelect) selectBody).getGroupByColumnReferences();
		...
		// this is the list of Table objects to join, we need to construct this list
		// from the list of joins formed above
		...
		// this Table variable stores the resultant table obtained from joining the
		// tables in the from clause
		Table resultTable = null;

		// if the number of tables to join is just 1, then we directly pass in the where
		// expression to the selection operation
		if (tablesToJoin.size() == 1) {
			// System.out.println("Only single table to join : " +
			// tablesToJoin.get(0).tableName);
			// in the case when I just have a single table with me to join, I just need to
			// filter that table and that will be my resultant table
			resultTable = WhereOperation.selectionOnTable(whereExpression, tablesToJoin.get(0));

			// after evaluating the resultant table that satisfies the where clause apply
			// aggregate operations on the table
		}
		...
		if (tablesToJoin.size() > 1) {
			...
			// this Expression variable stores the special skip condition that is when the
			// non join expression consists of both n1 and n2
			Expression tpch7SkipCondition = null;

			// the following is the ArrayList of Expression objects which are the non join
			// conditions used for filtering the tables
			...
			// the following is the modified list of tables that need to be joined, in this
			// case all the tables are filtered on a condition
			...
			for (Entry<Table, ArrayList<Expression>> etr : tableExpressionMap.entrySet()) {
				...
				if (etr.getValue().size() == 1) {
					...
					// this is the case where there is no filtering condition corresponding to a
					// table
			...
			/*
			 * HashMap<Integer, Table> mapOfTables = new HashMap<Integer, Table>(); int i =
			 * 0; for (Table table : filteredTablesToJoin) { mapOfTables.put(i, table); ++i;
			 * } // System.out.println(mapOfTables);
			 * 
			 * Table t1 = null; Table t2 = null; int countOfJoins = 0; int index =
			 * mapOfTables.size() - 1;
			 * 
			 * if (index > 0) { while (countOfJoins != index) { //
			 * System.out.println("In While"); for (int iterativeIndex = index - 1;
			 * iterativeIndex >= 0; iterativeIndex--) {
			 * 
			 * t1 = mapOfTables.get(index); t2 = mapOfTables.get(iterativeIndex); // if(t2==
			 * null) // System.out.println(t2); if (t2 == null) { continue; }
			 * 
			 * // this array list stores the join conditions for table involved
			 * ArrayList<String> arrayList = WhereOperation.evaluateJoinCondition(t1, t2,
			 * whereExpression); // System.out.println(arrayList.size()); for (String s :
			 * arrayList) // System.out.println(s);
			 * 
			 * if (arrayList.size() > 0 && mapOfTables.get(iterativeIndex) != null) { //
			 * call the HashJoin and evaluate the new table //
			 * System.out.println("In here"); // System.out.println("table to join :" +
			 * arrayList.get(0) + " : " + // arrayList.get(1)); resultTable =
			 * HybridHash.evaluateJoin(t1, t2, arrayList.get(0), arrayList.get(1),
			 * arrayList.get(2), swapDirectory); mapOfTables.put(iterativeIndex, null);
			 * mapOfTables.put(index, resultTable); countOfJoins++; } } } }
			 */
			// check if the tpch7 condition for the sub-query is not null and apply it to
			// the table
...
//			}
			// after evaluating the join that satisfies all the join conditions apply the
			// aggregate operations on the resultant table
		}
		...
		// the following is the list of select items, that is the items that need to be
		// computed and stored in the resulting table
		@SuppressWarnings("rawtypes")
		...
		// this variable is used to check if there is an aggregate function present in
		// the select list
		...
		/*
		 * // if there are no group by columns present and there are no aggregates then
		 * just project the items THIS CASE NEEDS TO BE HANDLED IN THE UPCOMING PROJECTS
		 * FOR THE CASE IF ORDERBY IS PRESENT OR NOT if(groupbyElementsList == null &&
		 * !aggregatePresent){ return ProjectTableOperation.projectTable(resultTable,
		 * selectList,false); } else{ // make the object of Aggregate class to call
		 * Aggregate functions AggregateOperations aggrObject = new
		 * AggregateOperations(); // this string is a comma separated string of GroupBy
		 * items String groupItems;
		 * 
		 * if(groupbyElementsList == null) groupItems = "NOGroupBy"; else groupItems =
		 * groupbyElementsList.toString();
		 * 
		 * // we store the select items list in an array String[] selectItemsArray =
		 * ((PlainSelect)selectBody).getSelectItems().toString().replaceAll("\\[",
		 * "").replaceAll("\\]", "").trim().split(",");
		 * 
		 * // test the select items array for(String s : selectItemsArray)
		 * System.out.println("select items array : " + s);
		 * 
		 * // call the aggregate function to get the resultant table resultTable =
		 * aggrObject.getAggregate(resultTable, selectItemsArray,
		 * groupItems,orderbyElementsList);
		 * 
		 * }
		 */
